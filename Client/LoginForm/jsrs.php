...
/**Parse a url string, return array of key=>value pairs
 * 
 * @param  string   $query     url query string (data sent to hidden iframe) 
 *                             Example:   "id=535&name=george"
...
function _urlDecode($query) {
  ...
  while ($i < count($a)) {
     ...
     $arr[htmlspecialchars(urldecode($b[0]))] 
...
// PHP version   :  Sebastien Cramatte [sebastien@webeclaireur.com] 
...
function jsrsReturn($payload) {
  ...
  Print (
      "<html><head></head><body style=\"width: 0; height: 0; position: absolute; margin: 50; visibility: hidden;\" onload=\"p=document.layers?parentLayer:window.parent;p.jsrsLoaded('" 
    . $C . "');\">jsrsPayload:<br>" 
