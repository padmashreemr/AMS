...
/**The top portion of this code is a wrapper class
 * that uses the lower portion to simplify communication
 * between a page and its hidden iframe element.  (So data can
 * be transferred from a database to a page, for example,
 * without having to refresh the page)
 *
 * The bottom portion of code, written by Brent Ashley,
 * is obtainable at  www.ashleyit.com/rs/main.htm
 *
 * The only portions of Brent Ashley's code that were
 * changed was the variable jsrsPost, which I set to 
 * false, so GET would be used instead of POST
 */

/**Communication object
 *
 * This object is used to send and receive data from
 * the hidden iframe.
 *
 * The send() function sends a URL query string to the
 * hidden iframe (through GET).  The hidden iframe's
 * src is "receive.php", so the data gets sent to
 * that document, to the php function receive();
 */

function comm() {
  this.toPage = "receive.php"; //send data to this php page
  this.callPHPFunction = "receive"; //php function to send data to
  this.callOnReturn = receive; //javascript function, called on data ready

  this.send = function(data) {
    // pass multiple parms as array
    this.iframeId = jsrsExecute(this.toPage, this.callOnReturn, 
              this.callPHPFunction, data);
  }
  
  //This function will return the javascript
  //array of keys from the hidden iframe
  this.getKeys = function() {
    var iframe = document.getElementById(c.iframeId);
    var doc = (jsrsBrowser == "IE" ) ? iframe.document 
              : iframe.contentDocument;
    return doc.keys;
  }

  //This function will return the javascript
  //array of values from the hidden iframe
  ...
  //This searches the javascript keys array
  //for the 'key' parameter, and then returns
  //the corresponding 'value' from the value array
  this.get = function(key) {
    var values = this.getValues();
    return values[this.getIndex(key)]; 
  }
